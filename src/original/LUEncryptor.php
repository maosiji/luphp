...
use MAOSIJI\LU\EXCEPTION\LUEncryptorException;
...
class LUEncryptor
{
     ...
     * @throws LUEncryptorException 加密失败时抛出异常
     */
    public function encrypt(string $data): string
    {
        ...
        // 检查加密是否成功
        if ($encrypted === false) {
            throw new LUEncryptorException( '加密失败：' . openssl_error_string() , LUEncryptorException::CODE_ENCRYPTION_FAILED );
        }
     ...
     * @throws LUEncryptorException 解密失败、数据无效时抛出
     */
    public function decrypt(string $encryptedData): string
    {
        ...
        if ($combined === false || strlen($combined) < $this->ivLength) {
            throw new LUEncryptorException( '无效的加密数据：Base64 解码失败或数据长度不足', LUEncryptorException::CODE_INVALID_DATA, null, $encryptedData, true );
        }
        ...
        // 检查解密是否成功
        if ($decrypted === false) {
            throw new LUEncryptorException( '解密失败'. openssl_error_string(), LUEncryptorException::CODE_DECRYPTION_FAILED, null, $encryptedData, true );
        }
    ...
    public function encryptBatch(array $dataArray): array
    {
        ...
        foreach ($dataArray as $data) {
            $results[] = $this->encrypt($data);
        }

        return $results;
    }

    public function decryptBatch(array $encryptedDataArray): array
    {
        $results = [];

        if ( empty($encryptedDataArray) || count($encryptedDataArray) <= 0) {
            return $results;
        }

        foreach ($encryptedDataArray as $data) {
            $results[] = $this->decrypt($data);
        }
     ...
     */
    public function _getCipherInfo(): array
    {
        return [
            'cipher' => $this->_getCipher(),
     ...
     */
    private function _getCipher(): string
    {
