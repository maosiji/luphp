...
class LUNo19 {
     ...
     * @param bool $sex         性别（false，女，默认；true 男）
     *
     * @return string           19位数字字符串，且第1位不为0
     */
    public function create( string $prefix='0755', int $pos=2, bool $sex=false ): string
    {
        $pos = ($pos<=2 || $pos>18) ? 2 : $pos;
        $prefix = ltrim($prefix, '0');

        $tool = new LURandom();
        if ( $sex ) { $sexNum = $tool->generateEven(1); }
        else { $sexNum = $tool->generateOdd(1); }
     ...
     */
    private function _create17Number( string $prefix ): string
    {
        $tool = new LURandom();
        // 第1位不能为0
        $first = strtoupper(substr($prefix, 0, 1));

        $number17 = $tool->generateNumber(16, true);
